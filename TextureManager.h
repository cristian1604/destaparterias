...
class TextureManager {
	...
	*/
	...
	/**
	* \brief descarga de memoria una textura cargada por el manejador
	* \param nombre del archivo de textura para descargar
	*/
	void UnloadTexture(const std::string &name);
	
	
	/**
	* \brief libera todas las texturas cargadas en el manejador
	*/
	void UnloadAll();
	
	private:
	static TextureManager *instance;			///< instancia unica de la clase
	std::map<std::string, sf::Image> textures;	///< mapa que almacena las imagenes con sus respectivos nombres de archivos
